...
test('workbench: rail, tabs, dock render correctly across nodes, themes and modes', async ({ page }) => {
  ...
  // Every tab renders without error for publish_executor (blocked, publish risk). W4 added I/O,
  // which is the only tab a DETERMINISTIC node like this one has anything to say on: its prompt and
  // its grants are both empty by construction.
  await expect(page.locator('.tabs button')).toHaveCount(11);
  await expect(page.locator('.tabs button', { hasText: 'I/O' })).toHaveCount(1);
  ...
  await expect(page.locator('.tabs button')).toHaveCount(11); // run still bound
  await cycleAllTabs(page);
