...
test('workbench: rail, tabs, dock render correctly across nodes, themes and modes', async ({ page }) => {
  ...
  // All 10 tabs render without error for publish_executor (blocked, publish risk).
  // W4 added "Default output" beside Schemas.
  ...
  await expect(page.locator('.tabs button')).toHaveCount(10); // run still bound (W4 added Default output)
  await cycleAllTabs(page);
