...
- Node behaviour is store-overlaid by default (`WORKSPACE_NODES_SOURCE=store`); topology is code.
- **`dryRun` is not a safety flag.** Every run record carries `dryRun: true` as a TypeScript literal (`workspace/executionTypes.ts:287`, set at creation in `executor.ts:456`); every run is created by `startDryRun()` — via `workflow_start_dry_run`, `site_duplicate`, the capture→clone chain and the conductor CLI (`node_execute` makes single-node runs the same way); no gate, guard or tool reads `run.dryRun`. A run created that way publishes to the live tenant site when `resolvePublishAuthority` and the gates pass. Never infer non-publishing safety from the word "dry", from the tool name, or from `publish_payload`'s `dryRun: true` envelope field (also a constant). The only inputs that stop a publish are: `operatorPublishDecision: "withheld"`, an absent authority (no operator approval and no autonomous snapshot), `<PREFIX>_PUBLISH_ENABLED=false`, `live: false` passed explicitly to `workflow_publish_run`, or a non-affirmative controller decision.
...
4. Publish authority = `resolvePublishAuthority(run)` only. Do not add caller flags that authorize publishing; do not add or remove a gate in `publisher.ts` (`PUBLISH_GATE_NAMES` is a closed set). In **engine code** `release_to_production` is called only by `release_executor` (`releaseExecution.ts`); that is not a system-wide guarantee — on the model path any node holding `project.call_tool` can name that verb and only the tenant's own tool policy (an external contract) and the prompt stand in the way, and the wire tool `project_call_tool` reaches it with no gate at all. Keep it that narrow; do not widen the set of nodes holding `project.call_tool`.
...
- That "operator" means a human: `workflow_set_operator_publish_decision` is callable by any full bearer and by every tenant's scoped chat bearer (`SITE_CLIENT_MANAGER_TOOLS`), by `runId` alone — the project pin does not apply to run-addressed tools (KNOWN_ISSUES K-M9). Whether a human stands behind that call is decided in `vreich-ui/platform`, not here.
- That the publish gates cover every route to a tenant: they cover `publishRun` (engine path and `workflow_publish_run`) and dispatch of publish-risk nodes; they do not cover `project_call_tool` (full bearer) or a model turn that holds `project.call_tool`.
- That `conductor-run` exists as a live Cloud Run job today: it is evidenced only by a Blobs-era runbook and an Aug-2026 cost note; no deploy artifact creates or syncs it.
