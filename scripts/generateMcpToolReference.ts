...
const CURATED: Record<string, Row> = {
  ...
  "workspace.update_node_dependencies": { effect: "mutate-workspace", storage: "workspace/current.json, changes/, revisions/", external: "-", autonomy: "caution", note: "Topology is PINNED to canonical at run time (overlayStoreNode); a store dependsOn edit does not change what runs, so this is REFUSED on a node canonical defines (T5). Store-authored nodes only." },
  "workspace.update_node_metadata": { effect: "mutate-workspace", storage: "workspace/current.json, changes/, revisions/", external: "-", autonomy: "caution", note: "Replaces the row's metadata field wholesale; the run-time overlay merges it per key with canonical. Since K-A9 (W5) this can no longer change a node's ROUTE — executionKind/route are stored fields this tool does not touch, changed only through workspace_update_node_execution. The legacy *Deterministic metadata flags still resolve a row that has no stored field." },
  "workspace.update_node_execution": { effect: "mutate-workspace", storage: "workspace/current.json, changes/, revisions/", external: "-", autonomy: "operator", note: "The ONE verb that changes how a node runs: model dispatch vs a named deterministic engine route. Turning publish_executor/release_executor to \"model\" makes a model turn responsible for the publish or release, with no idempotency ledger — see docs/KNOWN_ISSUES.md K-A9/K-A1." },
