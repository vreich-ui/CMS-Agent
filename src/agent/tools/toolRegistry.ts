...
export function createToolRegistry(): ToolDefinition[] {
  ...
  return [
    ...
    makeTool({ toolId:"monetize.ev_floor", name:"monetize.ev_floor", description:"Deterministic EV-floor arithmetic for an offer: floorUsd = this workflow's MEASURED projected run cost (the p50 of prior run totals from the node timing ledger, read server-side, never model-supplied; falling back to this run's accrued usage when there is no history) x floorMultiplier (default 1, break-even); expectedValueUsd = payoutUsd x conversionRate x estimatedVolume when all three are supplied (null, not fabricated, otherwise); meetsFloor compares the two and verdict states the conclusion. runCostBasis names where the cost came from and estimateBasis names whether the whole estimate rests on live data — only estimateBasis 'monetizer_data' — measured cost AND revenueBasis 'monetizer_data' AND volumeBasis 'tracking_engagement' (the property's measured sessions), with all three revenue figures present — is an EARNED block that may stop a run. A live payout multiplied by an invented traffic figure never reaches it. breakEvenConversions is always computable from payoutUsd alone. Never invents a run cost, a payout, or a conversion estimate.", inputSchema:z.object({ payoutUsd:z.number().positive().optional(), conversionRate:z.number().min(0).max(1).optional(), estimatedVolume:z.number().min(0).optional(), floorMultiplier:z.number().positive().optional(), runCostUsdOverride:z.number().min(0).optional(), revenueBasis:z.enum(["monetizer_data","stated_assumption"]).optional(), volumeBasis:z.enum(["tracking_engagement","stated_assumption"]).optional() }).strict(), outputSchema:schema, riskLevel:"read", sideEffect:"none", requiresApproval:false, timeoutMs:4000, category:"monetize", enabled:true, metadata:{}, handler: async(i,c)=>{ const d=z.object({payoutUsd:z.number().positive().optional(),conversionRate:z.number().min(0).max(1).optional(),estimatedVolume:z.number().min(0).optional(),floorMultiplier:z.number().positive().optional(),runCostUsdOverride:z.number().min(0).optional(),revenueBasis:z.enum(["monetizer_data","stated_assumption"]).optional(),volumeBasis:z.enum(["tracking_engagement","stated_assumption"]).optional()}).parse(i); let runCostUsd=d.runCostUsdOverride; let runCostBasis:RunCostBasis = runCostUsd===undefined ? "no_history" : "caller_override"; let runCostHistory:Awaited<ReturnType<typeof getRunCostEstimate>>["estimate"]|undefined; if (runCostUsd===undefined && c?.runId && c?.workflowId) { const projected=await getRunCostEstimate({ runId:c.runId, workflowId:c.workflowId, projectId:c.projectId }); runCostHistory=projected.estimate; if (projected.estimate.basis==="workflow_history") { runCostUsd=projected.estimate.estimatedRunCostUsd; runCostBasis="workflow_history"; } } if (runCostUsd===undefined && c?.runId) { const usage=await summarizeModelUsage({ runId:c.runId }); runCostUsd=usage.totalCostUsdEstimate; runCostBasis="accrued_run_cost"; } if (runCostUsd===undefined) { runCostUsd=0; runCostBasis="no_history"; } const runCostSource:"workflow_history"|"actual_run_usage"|"override"|"unavailable" = runCostBasis==="workflow_history"?"workflow_history":runCostBasis==="accrued_run_cost"?"actual_run_usage":runCostBasis==="caller_override"?"override":"unavailable"; return ok({ runCostSource, runCostHistory, ...computeEvFloor({ runCostUsd, payoutUsd:d.payoutUsd, conversionRate:d.conversionRate, estimatedVolume:d.estimatedVolume, floorMultiplier:d.floorMultiplier, runCostBasis, revenueBasis:d.revenueBasis, volumeBasis:d.volumeBasis }) }); } })
  ];
}
