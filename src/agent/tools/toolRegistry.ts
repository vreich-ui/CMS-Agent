import { z } from "zod";
import { invokeTenantReadTool, invokeTenantTool } from "./tenantInvoke.js";
...
import { resolveRouteEra } from "../workspace/routeRegistry.js";
import { captureCrawlStep, captureEmitStep, captureMapStep, captureScoreStep, captureThemeStep } from "../capture/captureEngine.js";
import { cloneIntakeStep, cloneMintStep, cloneRestampStep, cloneThemeBindStep } from "../capture/cloneEngine.js";
import { pdfTemplateIntakeStep, pdfTemplateMintStep, pdfTemplatePublishStep } from "../capture/pdfTemplateEngine.js";
import { TemplateLibraryStore } from "../library/templateLibraryStore.js";
import { instantiateLibraryTemplate } from "../library/templateInstantiate.js";
import type { TemplateLibraryObjectType } from "../library/templateLibraryTypes.js";
// T15.32 (#208; ADR-2026-08-25-structure-studio §5.2) — the per-tenant client-memory store. See
// clientMemoryStore.ts's own header for the tenancy seam and the determinism boundary this module
// respects: every write below is built from data already read back off the wire or the library, never
// from a model turn, and this tool file records the same discipline library.publish_template already
// keeps — code decides what to write, a model only ever supplies the ARGUMENTS to call the tool with.
...
// T15.32 (#208) — reads the id of the object object_instantiate_template/object_instantiate_section_template
// just created off the platform's OWN call result, never fabricated: the same objectId/object_id
// duality cloneEngine.ts's own result readers already accommodate (see e.g. its `row.object_id ??
// row.objectId`). Absent when the result carries neither shape — a caller that gets `undefined` back
// skips the memory write for that instantiation rather than recording a fabricated id.
const extractInstantiatedObjectId = (result: unknown): string | undefined => {
  if (!result || typeof result !== "object") return undefined;
  const record = result as Record<string, unknown>;
  const direct = record.objectId ?? record.object_id;
  if (typeof direct === "string" && direct) return direct;
  const nested = record.object;
  if (nested && typeof nested === "object") {
    const nestedId = (nested as Record<string, unknown>).id ?? (nested as Record<string, unknown>).objectId;
...
export function createToolRegistry(): ToolDefinition[] {
  ...
  return [
    ...
    // produced it — the existing (pre-fix) observations carry neither field. S-07 adds the run's
    // context.projectId (the CMS-Agent project id), so an observation produced by a run is partitioned
    // without the recording node knowing anything about tenancy. 2.6: timeoutMs raised
    ...
    // W5 (docs/plan/WORK-ORDER-2026-08-12-determinism.md §W5): the calculator still avoids invented
    // run cost by preferring qualified history, then actual accrued usage keyed by context.runId.
    // Its output is advisory only: authoritative stop decisions are built in economicDecision.ts
    // from separately verified cost, traffic and same-run offer receipts. runCostUsdOverride exists
    // only for callers with no runId in context (for example tests or a pre-run estimate).
    ...
    // spinning inside this tool's 25s window (project.call_tool's own cap is 30s). T12.13: those
    // calls go to the TARGET SITE'S CAPTURE BRIDGE, never to pdf-tool directly, and carry NO
    // credential at all — under Wolf's 2026-08-14 "option A, same-site writes" pdf-tool persists the
    // crawl output into its own store, so a tenant needs no per-site Netlify PAT to be captured and
    // there is no grant to fetch, forward, or leak. The bridge resolves the owning site, the canonical
    // pdf-tool project and the crawl's idempotency scope server-side; the completed snapshot.v1 is
    // read back through the bridge's own get_capture_snapshot (captureEngine.ts).
    makeTool({ toolId:"capture.crawl", name:"capture.crawl", description:"Create or poll the capture job for a source URL under the target project's capture policy (registry-resolved, deny-all default), through the TARGET SITE's own capture bridge. Without jobId: creates the job and returns {phase:'pending', jobState}. With jobId: polls once — 'pending' until terminal, then {phase:'completed', envelope} carrying the snapshot.v1, read back through the bridge's get_capture_snapshot. Never waits in-call; the long-run planes re-drive it. No storage credential is involved on either side: pdf-tool writes its own store (T12.13, option A), so no per-site Netlify PAT is needed and no grant, token, or site id can be returned, persisted, or logged.", inputSchema:z.object({ projectId:z.string().min(1), sourceUrl:z.string().url(), jobId:z.string().min(1).optional() }).strict(), outputSchema:schema, riskLevel:"write", sideEffect:"external_write", requiresApproval:false, timeoutMs:25_000, category:"capture", enabled:true, metadata:{ jobPlane:"target_capture_bridge", jobTools:["create_capture_job","get_capture_job_status","get_capture_snapshot"] }, handler: async (i) => { const d=z.object({projectId:z.string().min(1),sourceUrl:z.string().url(),jobId:z.string().min(1).optional()}).parse(i); const now=new Date().toISOString(); return ok(await captureCrawlStep({ targetProjectId:d.projectId, sourceUrl:d.sourceUrl, ...(d.jobId?{jobState:{jobId:d.jobId,status:"pending",attempts:0,createdAt:now,updatedAt:now}}:{}) })); } }),
    ...
    // T13.1 — the clone.* controlled tools: pure code, thin invocations of cloneEngine.ts (which
    // wraps the vendored platform clone.mjs — see provenance.ts) against stage artifacts. Registered
    // for the same reason capture.* is: node.get_effective_tools / node.prepare_execution /
    // node.get_effective_prompt all walk a node's allowedTools, and clone_conductor's four
    // deterministic-write nodes name these ids there. The deterministic route (cloneConductorRoutes.ts)
    // never actually dispatches through this registry on the happy path — it calls cloneEngine.ts's
    // step functions directly, with zero model calls — but toolExecutor.ts:49 returns "unknown_tool"
    // for anything absent from this registry, so leaving these four unregistered would mean the ONE
    // workflow built for a human to inspect shows tool names that resolve to nothing. clone.theme_bind
    // writes the palette through EXACTLY ONE sanctioned path — see its description and cloneEngine.ts's
    // own header for the law: set_site_brand_tokens is a PRIVILEGED op no agent may hand-author, and
    // this tool never constructs it or a set_site_fields-with-brandTokens patch.
    makeTool({ toolId:"clone.intake", name:"clone.intake", description:"Assembles the BOUNDED clone briefing the judgment nodes read: the target's LIVE component/page-type registries (section-type field names, not schemas), the site object's own brandTokens fetched with object_get, the captured theme's tokens, one compact shape record per page (route, source block shape, emitted section shape, gaps), and existing recipe summaries for reuse. Registries are read at call time and never mirrored. Bounded by construction — it measures its own output, targets 12k chars against the executor's 48k dependency bound, names anything it drops in budget.truncated, and refuses past its hard cap rather than shipping a silent excerpt. The raw snapshot, full mapping, emission report and full page bodies are deliberately absent; the deterministic stages hold a transport and fetch those themselves. Refuses when the named capture run is missing, incomplete, or belongs to a different target, when the target exposes zero or multiple active sites, or when the site carries no brandTokens.", inputSchema:z.object({ projectId:z.string().min(1), captureRunId:z.string().min(1) }).strict(), outputSchema:schema, riskLevel:"read", sideEffect:"none", requiresApproval:false, timeoutMs:20_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({projectId:z.string().min(1),captureRunId:z.string().min(1)}).parse(i); return ok(await cloneIntakeStep({ targetProjectId:d.projectId, captureRunId:d.captureRunId })); } }),
    makeTool({ toolId:"clone.mint", name:"clone.mint", description:"Re-validates recipe_designer's section_template/template designs against the target's LIVE component and page-type registries (vendored platform clone.mjs) and creates the survivors as governed DRAFT recipes, reuse-first. An unregistered section type, a slot the page type forbids, or a blueprint the type's schema cannot hold is REJECTED with the validator's own reason — never coerced.", inputSchema:z.object({ projectId:z.string().min(1), intake:anyObj, design:anyObj.optional() }).strict(), outputSchema:schema, riskLevel:"write", sideEffect:"external_write", requiresApproval:false, timeoutMs:25_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({projectId:z.string().min(1),intake:anyObj,design:anyObj.optional()}).parse(i); return ok(await cloneMintStep({ targetProjectId:d.projectId, intake:d.intake, design:d.design })); } }),
    makeTool({ toolId:"clone.theme_bind", name:"clone.theme_bind", description:"Writes theme_reconciler's reconciled tokens onto the capture run's own theme object, then applies that theme to the site through the site_apply_theme verb under a site checkout — the ONLY sanctioned palette writer. This tool never constructs set_site_brand_tokens or a set_site_fields-with-brandTokens op: brandTokens is forbidden under set_site_fields and set_site_brand_tokens is a PRIVILEGED op no agent may hand-author; site_apply_theme computes it itself, server-side. Applying is an EXACT REPLACE — when the proposal does not cover every color key the site already declares, the apply is REFUSED (theme_not_total) with no steps executed, never a partial write. If the target project's tool policy does not permit site_apply_theme, this refuses with that named reason before any lock is taken. Every checkout is released in a finally.", inputSchema:z.object({ projectId:z.string().min(1), intake:anyObj, themeProposal:anyObj.optional() }).strict(), outputSchema:schema, riskLevel:"write", sideEffect:"external_write", requiresApproval:false, timeoutMs:25_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({projectId:z.string().min(1),intake:anyObj,themeProposal:anyObj.optional()}).parse(i); return ok(await cloneThemeBindStep({ targetProjectId:d.projectId, intake:d.intake, themeProposal:d.themeProposal })); } }),
    makeTool({ toolId:"clone.restamp", name:"clone.restamp", description:"Re-assembles each mint survivor's captured pages onto its recipe through object_checkout -> object_patch(upsert_section) -> object_checkin, preserving every bound asset reference byte-identical — nothing here synthesizes, rewrites, or drops an asset src. A page whose recipe was rejected at mint is SKIPPED, never half-restamped; a page whose write fails at the wire is QUARANTINED, with its lock still released in a finally.", inputSchema:z.object({ projectId:z.string().min(1), intake:anyObj, mint:anyObj }).strict(), outputSchema:schema, riskLevel:"write", sideEffect:"external_write", requiresApproval:false, timeoutMs:25_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({projectId:z.string().min(1),intake:anyObj,mint:anyObj}).parse(i); return ok(await cloneRestampStep({ targetProjectId:d.projectId, intake:d.intake, mint:d.mint })); } }),
    // T15.34 (#210; ADR-2026-08-25-structure-studio §7) — the pdf_template.* controlled tools, same
    // reason and same posture as clone.* directly above: pure code, thin invocations of
    // pdfTemplateEngine.ts, registered ONLY so pdf_template_intake/_mint/_publish's allowedTools
    // resolve to something a human inspecting the workflow can read — the deterministic route
    // (cloneConductorRoutes.ts) never dispatches through this registry on the happy path, it calls
    // pdfTemplateEngine.ts's step functions directly with zero model calls. THESE CALL PDF-TOOL'S OWN
    // STORE, NEVER A CMS OBJECT VERB — see pdfTemplateEngine.ts's own header for why that is not a
    // second publish path.
    makeTool({ toolId:"pdf_template.intake", name:"pdf_template.intake", description:"Normalizes this run's initialInput.pdfTemplateBrief into a bounded, validated PDF-template entry list (name, renderer, tags, sample-data hints). Pure and total: an entry with no name, or a brief with no siteId, is named in rejectedEntries rather than thrown.", inputSchema:z.object({ initialInput:anyObj }).strict(), outputSchema:schema, riskLevel:"read", sideEffect:"none", requiresApproval:false, timeoutMs:5_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({initialInput:anyObj}).parse(i); return ok(pdfTemplateIntakeStep({ initialInput:d.initialInput })); } }),
    makeTool({ toolId:"pdf_template.mint", name:"pdf_template.mint", description:"Re-validates pdf_template_designer's proposals and creates the survivors as pdf-tool DRAFT templates (create_pdf_template), then — for every renderer except pdfme — runs validate_pdf_template and polls get_pdf_template_validation to a terminal report. A design with no usable templateJson, an unrecognized renderer, or a report that never reaches PASSED is REJECTED with pdf-tool's own reason, never coerced.", inputSchema:z.object({ projectId:z.string().min(1), intake:anyObj, design:anyObj.optional() }).strict(), outputSchema:schema, riskLevel:"write", sideEffect:"external_write", requiresApproval:false, timeoutMs:30_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({projectId:z.string().min(1),intake:anyObj,design:anyObj.optional()}).parse(i); return ok(await pdfTemplateMintStep({ targetProjectId:d.projectId, intake:d.intake, design:d.design })); } }),
    makeTool({ toolId:"pdf_template.publish", name:"pdf_template.publish", description:"Calls publish_pdf_template for every pdf_template_mint candidate that reached a validated state. Gating (operator veto / project autonomy policy) happens OUTSIDE this tool entirely, via the executor's generic publish-risk dispatch guard on this node's own riskLevel:\"publish\" — by the time this runs, the run's publish authority has already been checked.", inputSchema:z.object({ projectId:z.string().min(1), mint:anyObj }).strict(), outputSchema:schema, riskLevel:"publish", sideEffect:"external_write", requiresApproval:false, timeoutMs:25_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({projectId:z.string().min(1),mint:anyObj}).parse(i); return ok(await pdfTemplatePublishStep({ targetProjectId:d.projectId, mint:d.mint })); } }),
    // T15.31 (#207; ADR-2026-08-25-structure-studio §4.1) — the cross-tenant template library. See
    // src/agent/library/ for the full module set. clone_conductor's own deposit (publish_executor,
    // cloneConductorRoutes.ts) calls templateLibraryStore.ts's publish() directly, the same way
    // recipe_mint calls cloneMintStep directly — this tool exists for external/manual/future-workflow
    // callers (the workbench, an operator, #206/#208), not as clone_conductor's own dispatch path.
    makeTool({ toolId:"library.publish_template", name:"library.publish_template", description:"Deposits a minted section_template/template recipe into the cross-tenant template library (ADR-2026-08-25-structure-studio §4.1). Validates provenance is fully stateable (sourceUrl, captureRunId when clone-driven, the vendored capture-engine hashes, and the standards-pack version) and REFUSES, never coerces, an unstateable one with a named reason. A templateId not seen before mints version 1; content identical to the current latest version is a no-op returning that SAME version's record unchanged; content that differs mints the NEXT version, leaving every earlier version byte-identical — a published version is immutable.", inputSchema:z.object({ templateId:z.string().min(1), objectType:z.enum(["section_template","template"]), name:z.string().min(1), recipe:anyObj, sourceProjectId:z.string().min(1), sourceUrl:z.string().url().optional(), captureRunId:z.string().min(1).optional() }).strict(), outputSchema:schema, riskLevel:"write", sideEffect:"none", requiresApproval:false, timeoutMs:15_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({templateId:z.string().min(1),objectType:z.enum(["section_template","template"]),name:z.string().min(1),recipe:anyObj,sourceProjectId:z.string().min(1),sourceUrl:z.string().url().optional(),captureRunId:z.string().min(1).optional()}).parse(i); const result=await new TemplateLibraryStore().publish({ templateId:d.templateId, objectType:d.objectType, name:d.name, recipe:d.recipe as Record<string,unknown>, sourceProjectId:d.sourceProjectId, provenance:{ sourceUrl:d.sourceUrl, captureRunId:d.captureRunId, driven: d.captureRunId ? "clone" : "demand" } }); return ok(result); } }),
    makeTool({ toolId:"library.list_templates", name:"library.list_templates", description:"Lists cross-tenant template library records, optionally filtered by templateId, objectType, or a section type the recipe depends on (ADR-2026-08-25-structure-studio §4.1 point 4: discovery by structure kind + requirements, consumed by client-memory records and the workbench). Cross-tenant by construction — never filtered by which tenant originally minted a record; sourceProjectId is provenance, not an access scope (see templateLibraryTypes.ts).", inputSchema:z.object({ templateId:z.string().min(1).optional(), objectType:z.enum(["section_template","template","pdf_template"]).optional(), sectionType:z.string().min(1).optional() }).strict(), outputSchema:schema, riskLevel:"read", sideEffect:"none", requiresApproval:false, timeoutMs:15_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({templateId:z.string().min(1).optional(),objectType:z.enum(["section_template","template","pdf_template"]).optional(),sectionType:z.string().min(1).optional()}).parse(i); const templates=await new TemplateLibraryStore().list(d as { templateId?:string; objectType?:TemplateLibraryObjectType; sectionType?:string }); return ok({ templates }); } }),
    makeTool({ toolId:"library.instantiate_template", name:"library.instantiate_template", description:"Instantiates a cross-tenant library template into a target tenant, USING THE PLATFORM'S EXISTING object_instantiate_template / object_instantiate_section_template verbs — no new instantiation mechanism (ADR-2026-08-25-structure-studio §4.1: 'that would be the publish fork wearing a template's clothes'). Validates the template's section types against the TARGET's own live component registry, read fresh at call time; a type the target's platform build lacks is REJECTED with a capabilityBacklog entry, never coerced onto a near-neighbour type. An explicit operator withheld halts unconditionally (ADR-2026-08-25-publish-autonomy §2.4 rule 1); otherwise the target project's own tool-permission policy and the platform's own autonomy-aware approval floor (ADR-2026-08-25-structure-studio §4.2, reconciled server-side) decide the rest. On success, deterministically records the instantiation in the RECEIVING tenant's own client memory (ADR §5.2 — never the library's own sourceProjectId) so a later 'what structures do we have' question is answerable from memory alone.", inputSchema:z.object({ projectId:z.string().min(1), templateId:z.string().min(1), version:z.number().int().positive().optional(), operatorPublishDecision:z.enum(["approved","withheld"]).optional(), extraArgs:anyObj.optional() }).strict(), outputSchema:schema, riskLevel:"write", sideEffect:"external_write", requiresApproval:false, timeoutMs:25_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => {
      const d=z.object({projectId:z.string().min(1),templateId:z.string().min(1),version:z.number().int().positive().optional(),operatorPublishDecision:z.enum(["approved","withheld"]).optional(),extraArgs:anyObj.optional()}).parse(i);
      const store=new TemplateLibraryStore();
      const record = d.version ? await store.getVersion(d.templateId, d.version) : await store.getLatest(d.templateId);
      if (!record) return { ok:false, error:`No library record for templateId "${d.templateId}"${d.version ? ` version ${d.version}` : ""}.` };
      const outcome = await instantiateLibraryTemplate({ targetProjectId:d.projectId, record, operatorPublishDecision:d.operatorPublishDecision ?? null, extraArgs:d.extraArgs as Record<string,unknown> | undefined });
      // T15.32 (#208; ADR-2026-08-25-structure-studio §5.2) — "cross-tenant instantiations write to
      // the receiving client's memory." Deterministic: the record written is templateInstantiate.ts's
      // OWN library record (read back, never re-derived) plus the objectId the platform's OWN call
      // result just returned — no model involved, and this never runs when instantiation itself did
      // not succeed (a refusal is not something the receiving tenant now "owns").
      if (outcome.ok) {
        const instantiatedObjectId = extractInstantiatedObjectId(outcome.result);
        if (instantiatedObjectId) {
          const value: TemplateArtifactValue = { templateId: record.templateId, version: record.version, objectType: record.objectType, instantiatedObjectId, provenance: record.provenance };
          await new ClientMemoryStore().recordTemplates(outcome.targetProjectId, [value]);
        }
      }
      return ok(outcome);
    } }),
    // T15.32 (#208; ADR-2026-08-25-structure-studio §5.2) — the READ side of client memory. Exposed as
    // a plain read tool (riskLevel "read", no approval) so BOTH readers the ADR names can use the
    // identical surface: client_manager (an admin-chat question like "what page structures do we
    // have?"), and a copy workflow discovering what a tenant already owns before asking the studio for
    // a new one. Read-only, per §2's boundary: a reader may discover, never instantiate or mutate,
    // through THIS tool — instantiation stays library.instantiate_template's own, separate act.
    makeTool({ toolId:"client_memory.list_templates", name:"client_memory.list_templates", description:"Lists the finished templates recorded in ONE project's own memory namespace (ADR-2026-08-25-structure-studio §5.2) — never cross-tenant, unlike library.list_templates. Each entry names the templateId/version, the objectType, the object this project instantiated it as, and its full provenance, read back verbatim from the studio's terminal-stage write or a prior cross-tenant instantiation. An admin-chat question like \"what page structures do we have?\" is answerable from this alone.", inputSchema:z.object({ projectId:z.string().min(1) }).strict(), outputSchema:schema, riskLevel:"read", sideEffect:"none", requiresApproval:false, timeoutMs:5000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({projectId:z.string().min(1)}).parse(i); const templates = await new ClientMemoryStore().listTemplates(d.projectId); return ok({ projectId:d.projectId, templates }); } }),
    // 2026-09-08 — THE COST INPUT MOVES FROM "THIS RUN SO FAR" TO "WHAT THIS WORKFLOW ACTUALLY COSTS".
    //
    // W5 made this tool read a REAL cost instead of a model-supplied one, and the fabrication came back
    // anyway (run_1788769566432_5qnafb: estimatedRunCost 800, actual $3.86) because the node authored
    // the figure in its own output rather than calling this. Two changes close it:
    //   1. The conductor now PREFETCHES the figure into the node's input (costPrefetch.ts), so the
    //      model has a measured number whether or not it calls anything.
    //   2. This tool prefers that same measured projection. `monetize.ev_floor` runs at node 4 of 25,
    //      so the run's ACCRUED cost is a true number answering the wrong question — the floor asks
    //      what FINISHING costs. Order: caller override (tests / dry estimates only) -> this
    //      workflow's measured p50 run total -> this run's accrued usage. Each names its own basis on
    //      the artifact; a floor built on accrued-at-node-4 is a few cents and blocks nothing, which is
    //      the correct fail-open behaviour when there is no history.
    //
    // `revenueBasis` and `volumeBasis` are taken on the caller's word, and both are bounded: they can
    // only ever ESCALATE a block from advisory to earned, and only alongside a payout, a conversion rate
    // and a volume that all three make the floor miss. `volumeBasis` is now checkable against the
    // conductor-prefetched `trafficEstimate.basis` in the node's own input, which is the point — a claim
    // of "tracking_engagement" that the prefetch contradicts is visible in the run record. Follow-up
    // worth having (explicitly not this change): stamp both conductor-side, the way runCostBasis is.
    makeTool({ toolId:"monetize.ev_floor", name:"monetize.ev_floor", description:"Advisory deterministic EV arithmetic. The engine separately verifies same-run offer receipts, tenant, traffic window, qualified route-era cost evidence, currency, margin and cluster/override policy before persisting the only decision that may halt a run. Caller-supplied provenance labels on this calculator are descriptive and never authority.", inputSchema:z.object({ payoutUsd:z.number().positive().optional(), conversionRate:z.number().min(0).max(1).optional(), estimatedVolume:z.number().min(0).optional(), floorMultiplier:z.number().positive().optional(), runCostUsdOverride:z.number().min(0).optional(), revenueBasis:z.enum(["monetizer_data","stated_assumption"]).optional(), volumeBasis:z.enum(["tracking_engagement","stated_assumption"]).optional() }).strict(), outputSchema:schema, riskLevel:"read", sideEffect:"none", requiresApproval:false, timeoutMs:4000, category:"monetize", enabled:true, metadata:{ authoritative:false }, handler: async(i,c)=>{ const d=z.object({payoutUsd:z.number().positive().optional(),conversionRate:z.number().min(0).max(1).optional(),estimatedVolume:z.number().min(0).optional(),floorMultiplier:z.number().positive().optional(),runCostUsdOverride:z.number().min(0).optional(),revenueBasis:z.enum(["monetizer_data","stated_assumption"]).optional(),volumeBasis:z.enum(["tracking_engagement","stated_assumption"]).optional()}).parse(i); let runCostUsd=d.runCostUsdOverride; let runCostBasis:RunCostBasis = runCostUsd===undefined ? "no_history" : "caller_override"; let runCostHistory:Awaited<ReturnType<typeof getRunCostEstimate>>["estimate"]|undefined; if (runCostUsd===undefined && c?.runId && c?.workflowId) { const currentRouteEras=Object.fromEntries(listWorkspaceNodes().map((node)=>[node.id,resolveRouteEra(node)])); const projected=await getRunCostEstimate({ runId:c.runId, workflowId:c.workflowId, projectId:c.projectId, currentRouteEras }); runCostHistory=projected.estimate; if (projected.estimate.basis==="workflow_history") { runCostUsd=projected.estimate.estimatedRunCostUsd; runCostBasis="workflow_history"; } } if (runCostUsd===undefined && c?.runId) { const usage=await summarizeModelUsage({ runId:c.runId }); runCostUsd=usage.totalCostUsdEstimate; runCostBasis="accrued_run_cost"; } if (runCostUsd===undefined) { runCostUsd=0; runCostBasis="no_history"; } const runCostSource:"workflow_history"|"actual_run_usage"|"override"|"unavailable" = runCostBasis==="workflow_history"?"workflow_history":runCostBasis==="accrued_run_cost"?"actual_run_usage":runCostBasis==="caller_override"?"override":"unavailable"; return ok({ authoritative:false, runCostSource, runCostHistory, ...computeEvFloor({ runCostUsd, payoutUsd:d.payoutUsd, conversionRate:d.conversionRate, estimatedVolume:d.estimatedVolume, floorMultiplier:d.floorMultiplier, runCostBasis, revenueBasis:d.revenueBasis, volumeBasis:d.volumeBasis }) }); } })
  ];
}
