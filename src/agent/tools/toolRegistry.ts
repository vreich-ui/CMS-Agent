...
export function createToolRegistry(): ToolDefinition[] {
  ...
  return [
    ...
    makeTool({ toolId:"clone.intake", name:"clone.intake", description:"Assembles the clone workspace envelope: the finished capture run's artifacts (snapshot, mapping, theme, emission report) plus the target project's CURRENT object inventory and its LIVE component/page-type registries, read at call time and never mirrored. Refuses when the named capture run is missing, incomplete, or belongs to a different project.", inputSchema:z.object({ projectId:z.string().min(1), captureRunId:z.string().min(1) }).strict(), outputSchema:schema, riskLevel:"read", sideEffect:"none", requiresApproval:false, timeoutMs:20_000, category:"capture", enabled:true, metadata:{}, handler: async (i) => { const d=z.object({projectId:z.string().min(1),captureRunId:z.string().min(1)}).parse(i); return ok(await cloneIntakeStep({ targetProjectId:d.projectId, captureRunId:d.captureRunId })); } }),
