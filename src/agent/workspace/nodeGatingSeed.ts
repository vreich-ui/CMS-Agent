...
export const NODE_GATING_SEED: Record<string, NodeGatingSeedEntry> = {
  ...
  // W6b (2026-09-10) — the economic stop sits on reader_insight, the first paid stage after
  // monetization_strategy. reader_insight now depends on monetization_strategy, so the engine-owned
  // economicDecision always exists before this predicate is evaluated. A verified stop therefore
  // spends nothing on reader insight, research, brief, drafting, review or the publication tail.
  // Advisory/unavailable evidence does not fire and the graph proceeds normally.
  reader_insight: {
    skipWhen: [{
      when: "ev_floor_blocked",
      reason: "Run halted before reader_insight: the engine-owned economic decision verified a same-run, same-tenant below-floor result. No later paid model or write stage was invoked."
    }],
  ...
  brief_architect: {
    ...
    strategyPrefetch: true,
    rationale: "The aggression ceiling must exist before the brief that spends it is written; the client's editorial voice must be in hand for the same reason — the brief sets the tone guardrails every downstream writer reads."
  },
