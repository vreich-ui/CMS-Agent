...
export const captureConductorNodes = [
  ...
  {
    id: "capture_report",
    name: "Capture Run Report (terminal — human gate)",
    kind: "reporting",
    description: "Deterministic terminal assembly: rubric verdict, coverage delta, the never-released draft ledger, gaps by capability, the W10 evidence feed, and the adjudicator's human summary. The workflow ENDS here; publish/release require the human gate.",
    prompt: `Objective: assemble the terminal run report — rubric verdict, coverage delta, draft ledger (created/reused/quarantined/validation states), gaps grouped by missing capability, the W10 evidence feed (one entry per residual gap, carrying the adjudicator's disposition where present), and the human summary.\nOutput required: capture_run_report.v1 envelope. This node is the workflow's END: nothing downstream publishes, releases, builds, or deploys.\n${DETERMINISTIC_PROMPT_FOOTER}`,
    inputSchema: openInput,
    outputSchema: envelopeSchema("capture_run_report.v1", { rubric: { type: "object" }, drafts: { type: "object" }, w10EvidenceFeed: { type: "array" }, humanSummary: { type: "string" }, humanGate: { type: "object" } }),
    allowedTools: ["stage.get_output", "stage.list_outputs", "learning.record_observation"],
    assignedSkills: [],
    requiredInputs: ["capture_score", "gap_adjudicator"],
    produces: ["capture_run_report.v1"],
    riskLevel: "read",
    dependsOn: ["capture_score", "gap_adjudicator", "capture_emit_live"],
    status: "active",
    position: { x: 1980, y: 0 },
    updatedAt: UPDATED_AT,
