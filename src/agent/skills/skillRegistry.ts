...
import { CoalescedTtlCache, getBlobJson, getCmsAgentBlobStore, storeBackendLabel, type BlobStoreClient } from "../repository/blobs/blobClient.js";
...
export class MemorySkillRepository implements SkillRepository {
  ...
  // W7 — `options` is ignored here (the in-memory document IS the store) and honoured by
  // BlobSkillRepository's override. It exists so `mutate()` can demand a FRESH read: see the note
  // there, and BlobWorkspaceRepository.load(), which took the same shape for the same reason.
  protected async load(_options?: { fresh?: boolean }): Promise<SkillDocument> { return MemorySkillRepository.documents.get(this.backend)!; }
  ...
  // W7 — CLONED on the way out, like list()/listVersions()/getEvents() already were. Before the read
  // cache below, every load() re-scanned the store and each caller got a private object, so the
  // omission was invisible. With a cache, `skill_get` and `skill_resolve_for_node` handed the SAME
  // object to every caller for the life of the TTL, and one in-place edit anywhere would have
  // poisoned every read in that window — and could have been written back through mutate().
  async get(skillId: string) { const hit = (await this.load()).skills.find((s) => s.skillId === skillId); return hit ? structuredClone(hit) : undefined; }
  ...
  // W7 — READS FRESH, ALWAYS. `save()` rewrites every skill row and DELETES any key not in the
  // document it was handed, so this read-modify-write is the whole store's compare-and-swap even
  // though it has no version token to swap on. Served from the TTL cache it was a data-loss engine
  // on a multi-instance deployment: instance A caches the document, instance B deletes a skill, A
  // then handles any skill edit inside the TTL and writes its stale document back — resurrecting
  // B's deleted skill, and, in the other direction, deleting a skill B had just created. The cache
  // W1 added is for READS; this is not one.
  private async mutate(update: (doc: SkillDocument) => string | undefined, meta?: SkillMutationMeta, eventType = "skill.updated") { const doc = structuredClone(await this.load({ fresh: true })); assertVersion(doc, meta); const before = structuredClone(doc.skills); const skillId = update(doc); doc.skills = doc.skills.map(assertValidSkill); doc.skillVersion += 1; doc.updatedAt = now(); const changed = skillId ? doc.skills.find((s) => s.skillId === skillId) : undefined; if (changed && skillId) doc.versions.push({ skillId, versionId: makeId("version"), skillVersion: doc.skillVersion, createdAt: doc.updatedAt, summary: meta?.summary, skill: structuredClone(changed) }); doc.events.push({ id: makeId("event"), type: eventType, skillId, actor: actorLabel(meta), summary: meta?.summary, skillVersion: doc.skillVersion, beforeHash: hashValue(before), afterHash: hashValue(doc.skills), createdAt: doc.updatedAt }); await this.save(doc); return doc.skillVersion; }
  ...
  async getVersion(skillId: string, versionId: string) { const hit = (await this.load()).versions.find((v) => v.skillId === skillId && v.versionId === versionId); return hit ? structuredClone(hit) : undefined; }
...
export class BlobSkillRepository extends MemorySkillRepository {
  // W1 — every read used to re-scan three prefixes and open every blob under all of them, so the
  // cost of `skill_list` grew with every skill edit ever recorded (`skills/versions/`,
  // `skills/events/` are append-only). Measured live 2026-09-16: 20 s for a 63 KB answer, on the
  // critical path of a cold Workbench paint. One coalesced, short-TTL read per instance instead;
  // save() invalidates, so an operator's own edit is never served stale back to them.
  private readonly documentCache = new CoalescedTtlCache<SkillDocument>();
  ...
  protected override async load(options?: { fresh?: boolean }): Promise<SkillDocument> { if (options?.fresh) { this.documentCache.invalidate(); } return this.documentCache.read("document", () => this.scan()); }
  private async scan(): Promise<SkillDocument> { const listed = await this.store.list({ prefix: "skills/current/" }); if (!listed.blobs.length) { const doc = createDocument(); await this.save(doc); return doc; } const skills = (await Promise.all(listed.blobs.map((b) => getBlobJson<SkillDefinition>(this.store, b.key)))).filter(Boolean).map((s) => skillDefinitionSchema.parse(s)); const versionsList = await this.store.list({ prefix: "skills/versions/" }); const versions = (await Promise.all(versionsList.blobs.map((b) => getBlobJson<SkillVersionSnapshot>(this.store, b.key)))).filter(Boolean) as SkillVersionSnapshot[]; const eventsList = await this.store.list({ prefix: "skills/events/" }); const events = (await Promise.all(eventsList.blobs.map((b) => getBlobJson<SkillEvent>(this.store, b.key)))).filter(Boolean) as SkillEvent[]; return { schemaVersion: 1, skillVersion: Math.max(0, ...events.map((e) => e.skillVersion)), updatedAt: now(), skills, versions, events }; }
  protected override async save(doc: SkillDocument) { try { return await this.write(doc); } finally { this.documentCache.invalidate(); } }
  private async write(doc: SkillDocument) { const current = await this.store.list({ prefix: "skills/current/" }); const desired = new Set(doc.skills.map((s) => this.currentKey(s.skillId))); await Promise.all([...current.blobs.filter((b) => !desired.has(b.key)).map((b) => this.store.delete(b.key)), ...doc.skills.map((s) => this.store.setJSON(this.currentKey(s.skillId), s)), ...doc.versions.map((v) => this.store.setJSON(this.versionKey(v.skillId, v.versionId), v)), ...doc.events.map((e) => this.store.setJSON(this.eventKey(e.id), e))]); }
