...
export function createImprovementTools(deps: ImprovementToolDeps): WorkspaceTool[] {
  ...
  const ingestTrackingInput = z.object({ projectId: z.string().min(1), from: z.string().min(1), to: z.string().min(1), nodeId: z.string().min(1).optional(), ...mutationMeta }).strict();
  ...
  return [
    ...
    tool({ name: "feedback.ingest_tracking", description: `Outer-loop ingestion (T21.7, sibling of feedback.ingest_monetizer): pull the tracking sink's per-producer engagement rollups for a project/window and record each row as a feedback OUTCOME (source ${TRACKING_OUTCOME_SOURCE}, metrics ${TRACKING_METRIC_KEYS.join("/")}), so published-content engagement feeds optimizer.analyze. Rows are attributed to the producing nodeId/runId; pass nodeId to restrict the pull to one node. Requires the tracking sink connection (${TRACKING_SINK_URL_ENV} / ${TRACKING_SINK_TOKEN_ENV}). Best-effort and read-only against the sink; nothing external is written.`, zodSchema: ingestTrackingInput, inputSchema: objectSchema({ projectId: { type: "string", minLength: 1, description: "Tracking partition to read (the sink's TRACKING_PROJECT_ID)." }, from: { type: "string", minLength: 1, description: "Window start (ISO date or date-time)." }, to: { type: "string", minLength: 1, description: "Window end (ISO date or date-time)." }, nodeId: { type: "string", description: "Restrict to one producer node; also the attribution fallback for rows that omit one." }, ...metaJson }, ["projectId", "from", "to"]), execute: async (input) => {
      const data = ingestTrackingInput.parse(input);
      const stamped = meta(data);
      return ok({ result: await ingestTrackingRollups({ projectId: data.projectId, from: data.from, to: data.to, nodeId: data.nodeId, actor: stamped.actor }, { evaluationRepository }) });
    } }),
