...
export function createImprovementTools(deps: ImprovementToolDeps): WorkspaceTool[] {
  ...
  const feedbackRecordInput = z.object({ kind: z.enum(feedbackKinds), nodeId: z.string().min(1).optional(), runId: z.string().min(1).optional(), evalId: z.string().min(1).optional(), editDiff: z.unknown().optional(), outcome: z.unknown().optional(), note: z.string().min(1).optional(), projectId: z.string().min(1).optional(), ...mutationMeta }).strict();
  const listFeedbackInput = z.object({ nodeId: z.string().min(1).optional(), runId: z.string().min(1).optional(), kind: z.enum(feedbackKinds).optional(), projectId: z.string().min(1).optional(), limit: z.number().int().min(1).max(200).optional() }).strict();
  ...
  const ingestTrackingInput = z.object({ projectId: z.string().min(1), from: z.string().min(1), to: z.string().min(1), nodeId: z.string().min(1).optional(), cmsAgentProjectId: z.string().min(1).optional(), ...mutationMeta }).strict();
  ...
  const analyzeInput = z.object({ nodeId: z.string().min(1), from: z.string().datetime().optional(), to: z.string().datetime().optional(), projectId: z.string().min(1).optional() }).strict();
  ...
  return [
    ...
    tool({ name: "feedback.record", description: "Record human/analytics feedback: approve, reject, edit (with diff), or a published-performance outcome. Edit diffs are redacted before persistence. Pass projectId (the CMS-Agent project id) to stamp the record with the tenant it belongs to, so a project-scoped feedback.list can find it.", zodSchema: feedbackRecordInput, inputSchema: objectSchema({ kind: { type: "string", enum: [...feedbackKinds] }, nodeId: { type: "string" }, runId: { type: "string" }, evalId: { type: "string" }, editDiff: { type: "object" }, outcome: { type: "object" }, note: { type: "string" }, projectId: { type: "string", description: "CMS-Agent project id this record belongs to (e.g. \"dr-lurie\"). NOT the tracking sink's partition id." }, ...metaJson }, ["kind"]), execute: async (input) => {
    ...
    // S-07: projectId is filtered HERE rather than in EvaluationRepository.listFeedback because
    // matching an unstamped legacy record needs a run lookup, and the evaluation repository has no
    ...
    tool({ name: "feedback.ingest_tracking", description: `Outer-loop ingestion (T21.7, sibling of feedback.ingest_monetizer): pull the tracking sink's per-producer engagement rollups for a project/window and record each row as a feedback OUTCOME (source ${TRACKING_OUTCOME_SOURCE}, metrics ${TRACKING_METRIC_KEYS.join("/")}), so published-content engagement feeds optimizer.analyze. Rows are attributed to the producing nodeId/runId; pass nodeId to restrict the pull to one node. Requires the tracking sink connection (${TRACKING_SINK_URL_ENV} / ${TRACKING_SINK_TOKEN_ENV}). Best-effort and read-only against the sink; nothing external is written.`, zodSchema: ingestTrackingInput, inputSchema: objectSchema({ projectId: { type: "string", minLength: 1, description: "Tracking partition to read (the sink's TRACKING_PROJECT_ID)." }, from: { type: "string", minLength: 1, description: "Window start (ISO date or date-time)." }, to: { type: "string", minLength: 1, description: "Window end (ISO date or date-time)." }, nodeId: { type: "string", description: "Restrict to one producer node; also the attribution fallback for rows that omit one." }, cmsAgentProjectId: { type: "string", description: "CMS-Agent project id (e.g. \"dr-lurie\") to stamp on every recorded outcome, so a project-scoped feedback.list finds these rows. A DIFFERENT id from projectId above, which is the sink's own partition (e.g. \"drlurie\"); stamping that one would match no filter. Omit to record unstamped." }, ...metaJson }, ["projectId", "from", "to"]), execute: async (input) => {
      const data = ingestTrackingInput.parse(input);
      const stamped = meta(data);
      return ok({ result: await ingestTrackingRollups({ projectId: data.projectId, from: data.from, to: data.to, nodeId: data.nodeId, cmsAgentProjectId: data.cmsAgentProjectId, actor: stamped.actor }, { evaluationRepository }) });
    } }),
    ...
    tool({ name: "optimizer.analyze", description: `Evidence-cited diagnosis of a node: eval score aggregates, worst criteria, run failure codes, feedback counts, and (T21.22) an \`engagement\` block — the node's own ${TRACKING_OUTCOME_SOURCE} outcome records for the window aggregated and compared against the site median over the same window. The engagement block reports insufficient_data below ${ENGAGEMENT_MIN_SESSIONS} sessions and is absent entirely when the tracking sink (${TRACKING_SINK_URL_ENV} / ${TRACKING_SINK_TOKEN_ENV}) is unconfigured or unreachable.`, zodSchema: analyzeInput, inputSchema: objectSchema({ nodeId: { type: "string", minLength: 1 }, from: { type: "string", format: "date-time" }, to: { type: "string", format: "date-time" }, projectId: { type: "string", minLength: 1, description: "Tracking partition the site median is read from (the sink's TRACKING_PROJECT_ID); defaults to that env var." } }, ["nodeId"]), execute: async (input) => ok({ analysis: await analyzeNode(analyzeInput.parse(input), replayDeps) }) }),
    tool({ name: "optimizer.propose", description: "GEPA-style reflection: diagnose from eval evidence and propose a prompt mutation. PROPOSE-ONLY — nothing is applied until optimizer.promote. The proposal carries a named `cause` and its `rationale`: `engagement_below_site_median` when the node passes its rubric but underperforms the site median on measured reader behaviour (the failure a rubric cannot see), otherwise `rubric_criterion` or `no_evidence`.", zodSchema: proposeInput, inputSchema: objectSchema({ nodeId: { type: "string", minLength: 1 }, mode: modeJson }, ["nodeId"]), execute: async (input) => { const data = proposeInput.parse(input); return ok({ proposal: await proposeImprovement(data, replayDeps) }); } }),
