...
import { DEFAULT_EXECUTION_MODE, assessRunStall, getRun, listRuns, resetRun, retryNode, runModeSummary, runNextNode, startDryRun, summarizeRunForList, updateRunStatus } from "../../workspace/executor.js";
...
export function createWorkspaceTools(context: WorkspaceToolContext = {}): WorkspaceTool[] {
  ...
  return [
    ...
    tool({ name: "workflow.list_runs", description: "List compact dry-run workflow summaries. Node inputs/outputs, stage outputs, and artifact values are intentionally omitted; call workflow.get_run for one selected run. Each row carries a `mode` block naming what produced it and a `stall` block on status \"running\" rows naming whether the driver is alive.", zodSchema: listRunsInput, inputSchema: listRunsJsonSchema, execute: async (input) => { const runs = await listRuns(listRunsInput.parse(input), executionRepository); return ok({ runs: runs.map((run) => ({ ...summarizeRunForList(run), mode: runModeSummary(run), ...(assessRunStall(run) ? { stall: assessRunStall(run) } : {}) })) }); } }),
