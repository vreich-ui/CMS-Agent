...
// C2/C3 — CANDIDATE_SKILL_IDS_DESCRIPTION is shared verbatim by node.prepare_execution and node.execute
// so a caller reads the identical contract on both: candidate narrowing is one feature previewed and
// dispatched through two tools, not two features that happen to share a name. See nodeRuntime.ts's
// `scopeNarrowedSkillIds` (prepare) and `pinSkillSelection` (execute, via runSkillSelection.ts's C3
// section) for the enforcement itself — this description states the contract, it does not implement it.
const CANDIDATE_SKILL_IDS_DESCRIPTION = "Narrow this node's assigned skills to a subset for THIS call only — for a node whose assignment spans more than one job (e.g. reference_content_writer's faq_help_process / policy_explanation / evidence_story), names which job this dispatch is. NEVER WIDENS: an id the node was not assigned is refused and recorded (skillSelection.dropped, reason not_assigned on node.execute; simply absent from resolvedSkills on node.prepare_execution) — the node's own assignment stays the sole authority. Omit the field entirely for \"no recipe narrowing, use the node's full assignment (subject to scope)\", the same as every caller before this field existed; pass [] for \"this dispatch uses no skills\" — presence, not length, decides, so an empty array is a real instruction, not a no-op. On node.execute this narrows the PIN: a runId that already carries a pinned selection for this node (a reused runId, e.g. a retry) ignores candidateSkillIds exactly as it ignores every other input to pinning — write-once, first dispatch wins.";
const nodePrepareInput = z.object({ nodeId: z.string().min(1), input: z.unknown().optional(), dependencyOutputs: z.record(z.string(), z.unknown()).optional(), modelConfig: z.record(z.string(), z.unknown()).optional(), candidateSkillIds: z.array(z.string().min(1)).optional() }).strict();
const nodeExecuteInput = z.object({ nodeId: z.string().min(1), input: z.unknown().optional(), runId: z.string().min(1).optional(), dependencyOutputs: z.record(z.string(), z.unknown()).optional(), executionMode: z.enum(["mock", "openai"]).default(DEFAULT_EXECUTION_MODE), modelConfig: z.record(z.string(), z.unknown()).optional(), expectedWorkspaceVersion: z.number().int().nonnegative().optional(), candidateSkillIds: z.array(z.string().min(1)).optional() }).strict();
...
const nodeExecuteJsonSchema = objectSchema({ nodeId: { type: "string", minLength: 1 }, input: {}, runId: { type: "string" }, dependencyOutputs: { type: "object" }, executionMode: { type: "string", enum: ["mock", "openai"], default: DEFAULT_EXECUTION_MODE, description: EXECUTION_MODE_DESCRIPTION }, modelConfig: { type: "object" }, expectedWorkspaceVersion: { type: "integer", minimum: 0 }, candidateSkillIds: { type: "array", items: { type: "string", minLength: 1 }, description: CANDIDATE_SKILL_IDS_DESCRIPTION } }, ["nodeId"]);
const nodePrepareJsonSchema = objectSchema({ nodeId: { type: "string", minLength: 1 }, input: {}, dependencyOutputs: { type: "object" }, modelConfig: { type: "object" }, candidateSkillIds: { type: "array", items: { type: "string", minLength: 1 }, description: CANDIDATE_SKILL_IDS_DESCRIPTION } }, ["nodeId"]);
...
export function createWorkspaceTools(context: WorkspaceToolContext = {}): WorkspaceTool[] {
  ...
  const projectPolicyViews = async (projectId?: string): Promise<ProjectPolicyView[]> => {
    try {
      ...
      return configs.map((project) => ({ projectId: project.projectId, allowedTools: project.allowedTools, defaultToolPolicy: project.defaultToolPolicy, toolPolicies: project.toolPolicies, operatorToolPolicies: project.operatorToolPolicies }));
    } catch {
      return [];
    }
  };
