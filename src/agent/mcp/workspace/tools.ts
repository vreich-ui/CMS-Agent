...
export function createWorkspaceTools(context: WorkspaceToolContext = {}): WorkspaceTool[] {
  ...
  return [
    ...
    tool({ name: "workflow.get_run", description: "Get dry-run workflow execution state. The `mode` block reports what actually produced this run's outputs: executionMode, live (true only for real model output), and whether node definitions came from the static compile or the workspace store.", zodSchema: runIdInput, inputSchema: runIdJsonSchema, execute: async (input) => { const run = await getRun(runIdInput.parse(input).runId, executionRepository); return ok({ run: run ?? null, mode: run ? runModeSummary(run) : null }); } }),
    tool({ name: "workflow.list_runs", description: "List dry-run workflow executions. Each row carries a `mode` block naming what produced it, so a mock run is never mistaken for a real one.", zodSchema: listRunsInput, inputSchema: listRunsJsonSchema, execute: async (input) => { const runs = await listRuns(listRunsInput.parse(input), executionRepository); return ok({ runs: runs.map((run) => ({ ...run, mode: runModeSummary(run) })) }); } }),
