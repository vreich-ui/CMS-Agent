...
export function createWorkspaceTools(context: WorkspaceToolContext = {}): WorkspaceTool[] {
  ...
  const runStallTiming = async (workflowId: string): Promise<RunStallTimingContext> => {
    try {
      const aggregates = aggregateNodeTimingsByNode(await nodeTimingRepository.list({ workflowId }));
