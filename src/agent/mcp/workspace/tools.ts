...
export function createWorkspaceTools(context: WorkspaceToolContext = {}): WorkspaceTool[] {
  ...
  return [
    ...
    tool({ name: "workspace.get_nodes", description: "List workspace nodes — every governance-visible node across every registered workflow (publishing_conductor, capture_conductor, clone_conductor), not publishing_conductor's alone.", zodSchema: emptyInput, inputSchema: emptyJsonSchema, execute: async (input) => { emptyInput.parse(input); await workspaceRepository.ensureWorkspaceNodeSeeds(); return ok({ nodes: await workspaceRepository.getNodes() }); } }),
