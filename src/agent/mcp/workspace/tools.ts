...
const startDryRunInput = z.object({ projectId: z.string().min(1), input: z.any(), workflowId: z.string().min(1).optional(), executionMode: z.enum(["mock", "openai"]).default("mock"), entrypoint: z.enum(["article_body"]).optional(), articleBody: z.unknown().optional(), budgetUsd: z.number().nonnegative().optional() }).strict();
...
const runNodeJsonSchema = objectSchema({ runId: { type: "string", minLength: 1 }, nodeId: { type: "string", minLength: 1 }, dependencies: { type: "object" }, approved: { type: "boolean" } }, ["runId"]);
...
const nodeExecuteJsonSchema = objectSchema({ nodeId: { type: "string", minLength: 1 }, input: {}, runId: { type: "string" }, dependencyOutputs: { type: "object" }, executionMode: { type: "string", enum: ["mock", "openai"], default: "mock" }, modelConfig: { type: "object" }, expectedWorkspaceVersion: { type: "integer", minimum: 0 } }, ["nodeId"]);
...
export function createWorkspaceTools(context: WorkspaceToolContext = {}): WorkspaceTool[] {
  ...
  return [
    ...
    tool({ name: "workflow.run_node", description: "Run dependency-ready nodes; when nodeId is given, advance the run until that node completes (previously the parameter was accepted but ignored).", zodSchema: runNodeInput, inputSchema: runNodeJsonSchema, execute: async (input) => { const data = runNodeInput.parse(input); if (!data.nodeId) return ok({ run: await runNextNode(data.runId, { executionRepository, workspaceRepository, approved: data.approved }) }); let run = await getRun(data.runId, executionRepository); for (let i = 0; run && i < 100 && !HALTED_RUN_STATUSES.includes(run.status); i++) { run = await runNextNode(data.runId, { executionRepository, workspaceRepository, approved: data.approved }); const state = run.nodes.find((node) => node.nodeId === data.nodeId); if (state && state.status !== "queued" && state.status !== "running") break; } return ok({ run }); } }),
    tool({ name: "workflow.run_until", description: "Run dependency-ready nodes until the named node completes, then stop.", zodSchema: runUntilInput, inputSchema: runUntilJsonSchema, execute: async (input) => { const data = runUntilInput.parse(input); let run = await getRun(data.runId, executionRepository); for (let i=0; run && i<100 && !HALTED_RUN_STATUSES.includes(run.status); i++) { run = await runNextNode(data.runId, { executionRepository, workspaceRepository, approved: data.approved }); if (run.nodes.find((n) => n.nodeId === data.nodeId)?.status === "completed") break; } return ok({ run }); } }),
    tool({ name: "workflow.run_all", description: "Run all dependency-ready nodes, stopping before publish-risk nodes unless explicit approval exists.", zodSchema: runAllInput, inputSchema: runAllJsonSchema, execute: async (input) => { const data = runAllInput.parse(input); let run = await getRun(data.runId, executionRepository); for (let i=0; run && i<100 && !HALTED_RUN_STATUSES.includes(run.status); i++) run = await runNextNode(data.runId, { executionRepository, workspaceRepository, approved: data.approved }); return ok({ run }); } }),
