...
const nodeExecutionJsonSchema = objectSchema({ nodeId: { type: "string", minLength: 1 }, input: {}, runId: { type: "string" }, dependencyOutputs: { type: "object" }, executionMode: { type: "string", enum: ["mock", "openai"] }, modelConfig: { type: "object" }, expectedWorkspaceVersion: { type: "integer", minimum: 0 }, executionId: { type: "string" }, artifactType: { type: "string" }, from: { type: "string", format: "date-time" }, to: { type: "string", format: "date-time" } });
...
export function createWorkspaceTools(context: WorkspaceToolContext = {}): WorkspaceTool[] {
  ...
  return [
    ...
    tool({ name: "node.get", description: "Get a safe complete node inspection record.", zodSchema: nodeToolInput, inputSchema: nodeToolJsonSchema, execute: async (input) => ok({ node: await getNodeDetails(nodeToolInput.parse(input).nodeId, { workspaceRepository, executionRepository }) }) }),
    ...
    tool({ name: "node.prepare_execution", description: "Prepare one node execution without calling the model.", zodSchema: nodePrepareInput, inputSchema: nodeExecutionJsonSchema, execute: async (input) => ok({ preparation: await prepareNodeExecution(nodePrepareInput.parse(input), { workspaceRepository }) }) }),
    tool({ name: "node.execute", description: "Execute exactly one node independently from the full workflow.", zodSchema: nodeExecuteInput, inputSchema: nodeExecutionJsonSchema, execute: async (input) => ok(await executeNode(nodeExecuteInput.parse(input), { workspaceRepository, executionRepository })) }),
    tool({ name: "node.list_executions", description: "List node executions with filters (by runId/executionId/nodeId).", zodSchema: nodeQueryInput, inputSchema: nodeExecutionJsonSchema, execute: async (input) => ok({ executions: await listNodeExecutions(nodeQueryInput.parse(input), executionRepository) }) }),
    tool({ name: "node.get_latest_output", description: "Get latest node output with filters.", zodSchema: nodeQueryInput, inputSchema: nodeExecutionJsonSchema, execute: async (input) => ok({ output: (await listNodeOutputs(nodeQueryInput.parse(input), executionRepository))[0] ?? null }) }),
    tool({ name: "node.list_outputs", description: "List node outputs by node, run, execution, artifact type, or date range.", zodSchema: nodeQueryInput, inputSchema: nodeExecutionJsonSchema, execute: async (input) => ok({ outputs: await listNodeOutputs(nodeQueryInput.parse(input), executionRepository) }) }),
    tool({ name: "node.retry", description: "Retry a previous independent node execution.", zodSchema: nodeRetryInput, inputSchema: nodeExecutionJsonSchema, execute: async (input) => { const data = nodeRetryInput.parse(input); const run = await executionRepository.getRun(data.runId); const state = run?.nodes.find((node) => !data.nodeId || node.nodeId === data.nodeId); if (!run || !state) return ok({ execution: null }); return ok(await executeNode({ nodeId: state.nodeId, input: (state.input as any)?.input, dependencyOutputs: (state.input as any)?.dependencies, executionMode: run.executionMode ?? "mock" }, { workspaceRepository, executionRepository })); } }),
