...
import { DEFAULT_EXECUTION_MODE, assessRunStall, getRun, listRuns, resetRun, retryNode, runModeSummary, runNextNode, startDryRun, updateRunStatus } from "../../workspace/executor.js";
...
export function createWorkspaceTools(context: WorkspaceToolContext = {}): WorkspaceTool[] {
  ...
  return [
    ...
    tool({ name: "workflow.list_runs", description: "List dry-run workflow executions. Each row carries a `mode` block naming what produced it, so a mock run is never mistaken for a real one, and a `stall` block on status \"running\" rows naming whether the driver is alive.", zodSchema: listRunsInput, inputSchema: listRunsJsonSchema, execute: async (input) => { const runs = await listRuns(listRunsInput.parse(input), executionRepository); return ok({ runs: runs.map((run) => ({ ...run, mode: runModeSummary(run), ...(assessRunStall(run) ? { stall: assessRunStall(run) } : {}) })) }); } }),
