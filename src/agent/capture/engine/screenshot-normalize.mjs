 ...
 */
// T12.16 VENDORING DEVIATION (the ONE deviation from the upstream module — recorded in
// ../provenance.ts). It moved here from score.mjs when T12.10's normalization was re-vendored:
// upstream imports `sharp` at module top. CMS-Agent does not carry the sharp native dependency,
// and this runtime never holds screenshot binaries (the pdf-tool capture plane owns them), so
// every comparison resolves 'unavailable' before sharp is ever needed. The import is therefore
// lazy: scoring stays import-safe here, and a deployment that DOES materialize preview
// screenshots must add sharp to package.json before normalizeScreenshotPair can run. Everything
// below — the raster, the kernel, the flatten, the comparison — is byte-faithful in behaviour;
// COMPARISON_RASTER_KERNEL is stated as the literal 'lanczos3' (exactly the value of
// sharp.kernel.lanczos3) purely so the pin needs no module-top sharp.
async function loadSharp() {
  try {
    return (await import('sharp')).default;
  } catch {
    throw new ScreenshotNormalizeError(
      "Screenshot diffing requires the 'sharp' dependency, which this deployment does not install; visual comparisons are reported 'unavailable' unless preview screenshot binaries exist locally."
    );
  }
}
...
export const COMPARISON_RASTER_KERNEL = 'lanczos3';
