...
describe("RepositoryManager", () => {
  ...
  it("returns healthy memory repository health for every repository", async () => {
    ...
    await expect(manager.getRepositoryHealth()).resolves.toEqual({
      ...
      // W3.2.1: the durable tool execution ledger — every tenant call, model- or engine-invoked.
      toolExecution: { backend: "memory", readable: true, writable: true, version: "memory.v1" },
