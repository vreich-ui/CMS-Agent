...
describe("project.* MCP tools", () => {
  ...
  it("project.get returns safe connection metadata (booleans + env var names only)", async () => {
    ...
    expect(project.connection).toEqual({ endpointConfigured: true, tokenConfigured: true, mcpEndpointEnvVar: "DR_LURIE_MCP_ENDPOINT", tokenEnvVar: "DR_LURIE_MCP_TOKEN", endpointSource: "env", tokenSource: "env" });
