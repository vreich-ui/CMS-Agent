...
describe("project registry + Dr. Lurie definition", () => {
  ...
  it("project summary exposes only safe metadata, never the endpoint value or token", () => {
    ...
    expect(summary.connection).toEqual({ endpointConfigured: true, tokenConfigured: true, mcpEndpointEnvVar: "DR_LURIE_MCP_ENDPOINT", tokenEnvVar: "DR_LURIE_MCP_TOKEN", endpointSource: "env", tokenSource: "env" });
