...
describe("the snapshot read path (T12.13 part 3)", () => {
  ...
  it("refuses with capture_snapshot_unavailable when the bridge answers the read with an error, naming the bridge tool and its typed refusals", async () => {
    ...
    // T12.18 revises T12.13's "NAMED, not quoted": the failing tool is still named, and the
    // client's own structured refusal is now quoted alongside it — bounded and scrubbed — because
    // 29 byte-identical "returned an MCP error result" quarantines on 2026-08-19 proved a refusal
    // nothing can act on is a blind spot, not a safeguard. See describeMcpErrorResult's header.
    ...
    expect((error as CaptureRefusal).message).toContain('Capture job is "running", not complete');
  });
...
// -------------------------------------------------------------------------------------------------
// T12.18 — describeMcpErrorResult: what a client said when it refused, made legible without making
// it trusted. The live case this is pinned against: zilberman answered create_artifact_from_url
// with {statusCode: 400, error: "request_id must match req_<flow>_<topic>_<yyyymmdd>_<nn> ..."} and
// the engine reported "returned an MCP error result" 29 times, once per asset, saying nothing.
describe("describeMcpErrorResult", () => {
  it("quotes the client's structured error and leads with its status code", () => {
    const detail = describeMcpErrorResult({
      isError: true,
      content: [{ type: "text", text: "request_id must match req_<flow>_<topic>_<yyyymmdd>_<nn> using lowercase snake_case." }],
      structuredContent: { error: "request_id must match req_<flow>_<topic>_<yyyymmdd>_<nn> using lowercase snake_case.", statusCode: 400 }
    });
    expect(detail).toBe("status 400: request_id must match req_<flow>_<topic>_<yyyymmdd>_<nn> using lowercase snake_case.");
  });

  it("prefers a typed error_code over prose, and keeps both", () => {
    const detail = describeMcpErrorResult({
      isError: true,
      structuredContent: { error_code: "artifact_sha_mismatch", statusCode: 409, message: "expected sha256 did not match the fetched bytes" }
    });
    expect(detail).toBe("status 409 artifact_sha_mismatch: expected sha256 did not match the fetched bytes");
  });

  it("falls back to text content when the client sends no structured payload", () => {
    const detail = describeMcpErrorResult({ isError: true, content: [{ type: "text", text: "  upstream fetch failed  " }] });
    expect(detail).toBe("upstream fetch failed");
  });

  it("says so plainly rather than inventing detail when the client sends nothing", () => {
    expect(describeMcpErrorResult({ isError: true })).toBe("the client returned no error detail");
    expect(describeMcpErrorResult({ isError: true, structuredContent: { statusCode: 500 } })).toBe("status 500");
  });

  it("keeps a sha256 digest readable — a mismatch is exactly what this exists to explain", () => {
    const sha = "6e38852fcf28ab665695862e8372906974133bce8fda6990d3cf7a2b4064ee7f";
    expect(describeMcpErrorResult({ isError: true, structuredContent: { error: `sha256 ${sha} did not match` } })).toContain(sha);
  });

  it("redacts credential-shaped runs and flattens newlines so remote text cannot forge structure", () => {
    const detail = describeMcpErrorResult({
      isError: true,
      structuredContent: { error: "auth failed\nAuthorization: Bearer nfp_radioactive_do_not_persist\ntoken=hunter2hunter2" }
    });
    expect(detail).not.toContain("nfp_radioactive_do_not_persist");
    expect(detail).not.toContain("hunter2hunter2");
    expect(detail).not.toContain("\n");
    expect(detail).toContain("[redacted]");
  });

  it("caps length so a remote party cannot flood run state", () => {
    const detail = describeMcpErrorResult({ isError: true, structuredContent: { error: "x".repeat(5000) } });
