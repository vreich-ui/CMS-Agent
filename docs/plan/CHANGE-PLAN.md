...
**R-6 ✅ Retire `article_body_validate` / `article_body_get_schema`.** Landed 2026-07-30 — removed outright (not proxied), together with R-23's recommended delete: both legacy `{schema_version, nodes}` schemas in `store.ts` (Zod `articleBodySchema` and `articleBodyJsonSchema`) and every consumer — the two wire tools (manifest 137 → 135), the conductor run-context copy (now the article_body node's OWN `outputSchema`), `project_validate_handoff`'s legacy pick (same authority now), the UI validator/schema panels, and the legacy shape in the `publish_build_payload` / `publish_validate_payload` / `workflow_publish_readiness` input schemas. The client's `object_validate` remains the authority beyond the node's envelope. Bonus fix while locking this down: the generic output validator ignored `minProperties`/`maxProperties`, so `body: {}` passed the node's schema; it enforces them now.
...
Recommended: delete #2 (it silently overrides the seeded schema on every fresh workspace), resolve #1 via R-22, and rename the artifact to describe "one client object". → *Delete of #2 landed 2026-07-30 with R-6: both `store.ts` forms of the monolith and every consumer are gone (see R-6). The artifact rename is the half that remains open, pending a first-hand contract read.*
