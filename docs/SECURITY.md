...
| Layer | Control | Gap |
|---|---|---|
| MCP bearer | static / OAuth = full catalog; scoped = allowlist + project pin | No per-tool RBAC for full bearers; OAuth has one scope; `x-workspace-actor` is self-asserted attribution (K-M4) |
...
| Publish gates | five closed gates + publish-risk dispatch gate + operator veto; policy snapshot per run | when `publish_executor` runs the **model path** (canonical default), `publishRun`'s gates are not consulted (K-A1) |
...
| `release_executor` / `release_to_production` | triggers a production build | idempotency ledger; only the releaser node speaks the verb |
| `project_call_tool` on a full-access project | reaches publish/deploy/commerce/member tools on the tenant; `wipe_blob_stores` is the only default `needs_approval` | project policies |
