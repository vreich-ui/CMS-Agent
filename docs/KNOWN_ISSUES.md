...
- Scenario: cosmetic in the UI (a handful of same-millisecond records display in the wrong order), but a `limit`-capped page can drop the genuinely newest record of a tied group.
- **Correction to this entry.** It prescribed "a tiebreak on the monotonically-increasing record id". The ids were not monotonically increasing: `makeImprovementId` was `${prefix}_${Date.now()}_${random}`, so inside one millisecond the only varying part was the random tail and tiebreaking on the id would have been tiebreaking on a coin flip. It would have LOOKED deterministic, which is worse than the stable-sort behaviour it replaced. The entry named the right mechanism and assumed a property the ids did not have.
- Also understated: the blob repositories do not sort insertion order at all. They sort whatever order the store listed the keys in, so a tied group came back in an order CMS-Agent does not control — not reliably oldest-first, just arbitrary.
- **Fixed (ops/tick-deploy-and-hygiene).** Two halves, neither sufficient alone. `makeImprovementId` now carries a per-process sequence that resets when the clock advances (`${prefix}_${ms}_${seq36}_${random}`), so ids increase in creation order within a millisecond and stay unique across processes. `src/agent/repository/newestFirst.ts` is now the one sort order for all four repositories (Memory/Blob × Evaluation/Improvement), tying on the record's OWN identity field rather than on an id it merely references. `tests/agent/repository/newestFirstTies.test.ts` writes five records under a frozen clock and asserts `limit: 3` returns the three newest by insertion, in the blob case against a store that deliberately lists keys backwards.
