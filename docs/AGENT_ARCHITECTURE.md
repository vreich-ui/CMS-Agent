...
`learning.record_observation` (controlled tool and MCP tool) stamps `runId`/`nodeId` from the executing context into both top-level fields and `metadata` (`store.ts:594-604`), so an observation cannot forge its origin. `learning_recorder` (tail) records structured observations and improvement candidates; `recordTerminationObservations` (`executor.ts:1123`) records blocked/failed/completed outcomes; `publishRun` records publish executed/failed. Observations are soft-deleted only. Node outputs carry `provenance{promptVersion, model, capturedAt}` stamped at execution.
