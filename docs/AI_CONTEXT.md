...
CMS-Agent is a TypeScript (Node 22 in Docker/CI, `.nvmrc` says 24; ESM, `tsx`, zod 4, `@openai/agents`, `@modelcontextprotocol/sdk`) workspace and orchestration service for content workflows. It exposes one MCP Streamable-HTTP endpoint (163 tools) and runs multi-node agent workflows ("conductors") that write, review, materialize media for and publish content into tenant sites. Tenant sites are separate Netlify-hosted MCP servers from `vreich-ui/platform`; they own published content. CMS-Agent owns workflow definitions, run state, evaluations and learning state.

Production: Google Cloud Run service `cms-agent-mcp` (project `cms-agent-503015`, `us-central1`; live 2026-09-06, `/health` → `store: gcs`) + Cloud Run jobs (`continuation-tick` every 2 min — created by runbook, image synced by the trigger; `site-credential-reconciler` daily — deploy scripts in repo; `conductor-run` on demand — **created by hand in Aug 2026, not synced, current existence unverifiable from the repo**), all reading and writing one GCS bucket `cms-agent-503015-cms-agent-state` (`WORKSPACE_STORE=gcs`). The service image is `Dockerfile.mcp` (ENTRYPOINT `mcpServerMainRun.ts`); `Dockerfile`'s ENTRYPOINT is the conductor job; jobs override the command per job. Netlify hosts two static SPAs (`ui/`, `workbench/`) that call the Cloud Run `/mcp` directly with a pasted bearer — **and still deploys and routes all 8 `netlify/functions/*`** (probe 2026-09-06: `/api/mcp` 502, `/api/agent` 502, `/api/session` 401 alive, the OAuth authorization server 200 alive — its tokens live in Netlify Blobs and cannot authenticate to Cloud Run). Treat every function except `session` as dead-but-routable legacy.
...
5. Publish authority is `resolvePublishAuthority(run)` (`workspace/publishDecision.ts`): `run.operatorPublishDecision === "approved"` **or** `run.publishingPolicySnapshot.autonomyMode === "autonomous"` (snapshotted once at run creation, `executor.ts:483-490`, never re-read); `withheld` always blocks. Caller flags (`approved: true`) are inert. `publishRun` (`workspace/publisher.ts`) applies five closed gates: `operator_enabled`, `publish_authorized`, `explicit_live`, `operator_not_withheld`, `controller_decision_go`. Note `explicit_live` passes unless the caller sends `live: false` (`publisher.ts:157`, `input.live !== false`) — omitting `live` is a live publish, whatever the tool description says (KNOWN_ISSUES D-13). **Who may set the operator decision:** any full bearer, and every tenant's scoped chat bearer, by `runId` with no project pin (K-M9). **What the gates do not cover:** the wire tool `project_call_tool` and any model turn holding `project.call_tool` — see PUBLISHING_ARCHITECTURE §2.0 for the complete path list and the exact conditional invariant.
6. Whether `publish_executor` runs the engine path (`publishRun`) or a model turn depends on the **store row's** `metadata.publishExecutorDeterministic` (`"execute"` / `true` / absent); canonical `nodes.ts` sets none. The same is true of every other `…Deterministic` flag the executor reads (`releaseExecutorDeterministic`, `publishPayloadDeterministic`, `publicationControllerDeterministic`, `contractIntelligenceDeterministic`, `artifactMaterializerDeterministic`, `learningRecorderDeterministic` — `executor.ts:1179-1185`): canonical sets most of them `true`, but the store row's value wins per key (`executor.ts:358`), `store:update` restores none of them (only `--set-publish-executor-mode` touches one), and a row set to `false` turns that node into a model turn holding whatever tools it has — for `release_executor` that includes `project.call_tool`. `learning_recorder`'s flag is set by no canonical literal, so by default its observations are a model's self-report, not a templated fact record. The repo cannot tell what production does; query `workspace_get_node` for each tail node.
...
- Never add a publish gate or a caller-side authority flag; never let a node grant satisfy a gate; in engine code only `release_executor` may call `release_to_production` (on the model path the tenant's tool policy — an external contract — and the prompt are the only enforcement; `composeWorkflowNodes`' `PUBLISH_ONLY_VERBS` check looks for tenant verbs in `allowedTools`, which never holds them, so it does not see `project.call_tool` grants — `publishingTail.ts:207`).
...
- that a `dryRun` run is safe — **`dryRun: true` is a literal on every run and nothing reads it** (`executionTypes.ts:287`, `executor.ts:456`); every run comes from `startDryRun()` (`workflow_start_dry_run`, `site_duplicate`, capture→clone, conductor CLI) and publishes live when the gates pass; `workflow_run_all` publishes under autonomous policy or an operator decision;
...
| `src/agent/mcp/workspace/tools.ts` + `agentTools/changesTools/constellationTools/improvementTools/siteDuplicationTools/siteCredentialTools/visualIdentityTools.ts` | the 163 tools (zod + JSON Schema + execute) | adding/changing a tool |
...
## 7e. External contracts — do not treat as local truth

Statements here about tenant MCP servers (`vreich-ui/platform`), PDF-Tool, the tracking sink (`kugel-data`), the Monetizer MCP, Netlify and Google Cloud describe what CMS-Agent's code *expects* of them (client adapters, knowledge prompts, captured fixtures, deploy scripts). None is verified in this repository; the cross-repository audit owns them. In particular: what the tenant does with its scoped chat bearer, whether a human stands behind an approval, what `release_to_production` really does, and whether the Cloud Run jobs named in the runbooks exist today.
...
Reproductions of the cheaply reproducible defects (C-1, C-3, C-12, K-M4, K-M5, K-M9) run offline: `WORKSPACE_STORE=memory npx tsx scripts/repro/knownIssues.ts`. The MCP reference check also refuses stale hand-quoted tool counts: `WORKSPACE_STORE=memory npx tsx scripts/generateMcpToolReference.ts --check`.
