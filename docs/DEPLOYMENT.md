...
6. Between deploys: `npm run check:job-images` (or the `check-job-images` action of `cloud-run-plane.yml`, which also runs daily on a schedule) reports any plane whose image digest differs from the service's **any listed plane that does not exist in the project at all**, and **any job running the mcp-service image that the list does not name**, exiting non-zero on any of the three. The second direction was added on 2026-09-08 (K-O3): an absent plane used to be a note, so two jobs sat unbuilt while this check went green every morning. The third closes the remaining half of C-10 — a plane the project runs and the list has never heard of is pinned by nothing, and that is the 2026-09-07 incident seen from the other side. An UNLISTED plane fails the deploy as well as the check, unlike an ABSENT one: a job that does not exist cannot run stale code, and a job that does can. Read-only; it never repins and never creates. The same workflow runs `npm run env:audit` as a separate job, which needs the `NETLIFY_AUTH_TOKEN` Actions secret and fails rather than skipping when it is absent.
...
| `SITE_CREDENTIAL_RECONCILER_GCP_PROJECT`, `_REGION`, `_JOB` | `mcp/workspace/siteCredentialTools.ts` | lets `site_credentials_apply` fire the job | — / — / `site-credential-reconciler` | `_GCP_PROJECT`/`_REGION`: `scripts/deploy-service.sh` (`ENV_PAIRS`, derived from that deploy's own `PROJECT`/`REGION` — KNOWN_ISSUES C-13, fixed); `_JOB`: none, falls back to its default |
