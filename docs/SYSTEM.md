...
This repository owns the workspace, workflow runs, receipts, evaluations, playbooks, learning observations, the project (tenant) registry and the tenant chat credential — never published content, artifact bytes, deploy state or tracking data. The full authority matrix is `docs/system/SYSTEM-AUTHORITY-MATRIX.md`; the contracts this repository produces or consumes are in `docs/system/SYSTEM-CONTRACTS.md`; cross-repository defects that involve this repository are in `docs/system/SYSTEM-KNOWN-ISSUES.md`.
